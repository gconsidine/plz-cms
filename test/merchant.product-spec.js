...
describe('merchant.product | Public API', function () {
  ...
  describe('plz.add.product()', function () {
    before(function (done) {
      plz = require('../app/core.hub')(Tc.validMerchantConfig);
    ...
    it('should return error if required fields are missing', function(done) {
      ...
      plz.add.product(invalidRequest, function (error) {
        error.should.be.true;
        var requestWithoutCart = {
          to: 'invalid target'
        };
        plz.add.product(requestWithoutCart, function (error) {
          error.should.be.true;
          done();
        });
      });
    ...
    it('should modify given target to contain the product', function(done) {
      var cart = [];
      var addRequest = {
        to: cart,
        name: 'Acme 16-oz Claw Hammer',
      ...
      plz.add.product(addRequest, function (error, result) {
        error.should.be.false;
        result.should.not.equal({});
        var cartEntry = result[addRequest.name];
        cartEntry.quantity.should.equal(1);
        for(var field in cartEntry) {
          if(Tc.validProduct.hasOwnProperty(field) && field !== "_id") {
            if(cartEntry[field] instanceof Array){
              var arrayString = cartEntry[field].toString();
              arrayString.should.equal(Tc.validProduct[field].toString());
            }
            else{
              cartEntry[field].should.equal(Tc.validProduct[field]);
            }
          }
        }
        done();
    ...
    it('should add quantity if already present in target', function(done) {
      var cart = [];
      var addRequest = {
        to: cart,
        name: 'Acme 16-oz Claw Hammer',
      ...
      plz.add.product(addRequest, function (error, result) {
        error.should.be.false;
        result.should.not.equal({});
        result[addRequest.name].quantity.should.equal(1);
        plz.add.product(addRequest, function (error, result) {
          error.should.be.false;
          result.should.not.equal({});
          result[addRequest.name].quantity.should.equal(2);
          done();
        });
      });
    });
